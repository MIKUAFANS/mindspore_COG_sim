...
class CogEnvDecoder:
    def __init__(self, worker_id=1, train=True, allow_multiple_obs=True, env_name=None, mat_num=0,
                 windows_width=600, windows_height=300, no_graphics=False, time_scale=1, seed=0, force_sync=True):
        if env_name == None:
            env_name = os.path.dirname(__file__) + "/linux_v3.1/cog_sim2real_env.x86_64"
        if train:
    ...
    def reset(self, fri_cor=0.1, KP=8, KI=0, KD=2, VK1=0.375, M=3.4, Wheel_I=0.0125, obs_mat=False):
        ...
        # return state
        # return vector_state
        return np.array(self_pos + goal1 + goal2 + goal3 + goal4 + goal5), obs
    ...
    def step(self, action):
        ...
        # state = {"color_image": img, "laser": laser, "vector": vector_state}
        state = np.array(self_pos + goal1 + goal2 + goal3 + goal4 + goal5)

        # return state, reward, done, [info, judge_result]
        return state, reward, done, obs, flag_ach
        # return vector_state, reward, done, [info, judge_result]
