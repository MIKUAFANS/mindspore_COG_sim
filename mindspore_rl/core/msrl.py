...
class MSRL(nn.Cell):
    ...
    def __create_environments(self, config, num_agent=1):
        ...
        if self.num_collect_env > 1:
            # collect_env = CogEnvDecoder(env_name="/home/lx/COG-sim2real-challenge-main/linux_v3.1/cog_sim2real_env.x86_64", no_graphics=True, 
            #         time_scale=1, worker_id=2, seed=1234, force_sync=True)
            # collect_env = GymEnvironment(config['eval_environment']['params'],env_id=1)
            collect_env = CogEnvironment(config['collect_environment']['params'],env_id=1)
            print('1 is ok')
            # eval_env = CogEnvDecoder(env_name="/home/lx/COG-sim2real-challenge-main/linux_v3.1copy/cog_sim2real_env.x86_64", no_graphics=True, 
            #         time_scale=1, worker_id=3, seed=1234, force_sync=True)
            # eval_env = GymEnvironment(config['eval_environment']['params'],env_id=2)
            eval_env = CogEnvironment(config['eval_environment']['params'],env_id=2)
        else:
            #collect_env = CogEnvDecoder(env_name="/home/lx/COG-sim2real-challenge-main/linux_v3.1/cog_sim2real_env.x86_64", no_graphics=True, 
                    #time_scale=1, worker_id=2, seed=1234, force_sync=True)
            # collect_env = GymEnvironment(config['eval_environment']['params'],env_id=1)
        ...
        #print(collect_env.action_space)
        return collect_env, eval_env
    ...
    def __create_policy_and_network(self, config):
        ...
        if params:
            ...
            if not params.get('action_space_dim'):
                config['policy_and_network']['params'][
                    'action_space_dim'] = collect_env.action_space.shape[-1]
