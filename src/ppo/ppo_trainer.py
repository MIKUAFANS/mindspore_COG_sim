...
class PPOTrainer(Trainer):
    ...
    def train_one_episode(self):
        ...
        while self.less(j, self.duration):
            reward, new_state, action, miu, sigma, done = self.msrl.agent_act(
        ...
        replay_buffer_elements = self.msrl.get_replay_buffer_elements(
            transpose=False, shape=(1, 0, 2))
        state_list = replay_buffer_elements[0]
        state_list = state_list.reshape(1, state_list.shape[0], state_list.shape[1])
        action_list = replay_buffer_elements[1]
        action_list = action_list.reshape(1, action_list.shape[0], action_list.shape[1])
        reward_list = replay_buffer_elements[2]
        reward_list = reward_list.reshape(1, reward_list.shape[0], reward_list.shape[1])
        next_state_list = replay_buffer_elements[3]
        next_state_list = next_state_list.reshape(1, next_state_list.shape[0], next_state_list.shape[1])
        miu_list = replay_buffer_elements[4]
        miu_list = miu_list.reshape(1, miu_list.shape[0], miu_list.shape[1])
        sigma_list = replay_buffer_elements[5]
        sigma_list = sigma_list.reshape(1, sigma_list.shape[0], sigma_list.shape[1])
